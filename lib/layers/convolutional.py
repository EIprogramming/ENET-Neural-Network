...
class Convolutional(Layer):
    def __init__(self, input_shape: tuple | None, kernel_params: tuple[int, int, int] | None, P: int = 1, **kwargs):
    ...
    def convolve(self, inputs):
        print(f"Inputs Shape: {inputs.shape}\nFilters Shape: {self.kernels.shape}\nOutput shape: {self.output_shape}")
        for i in range(0, self.output_shape[0]):
            for j in range(0, self.output_shape[1]):
                for k, kernel in enumerate(self.kernels):
                    ...
                    print(f"Operands: {inputs_slice.shape}, {kernel.shape}")
    ...
    def convolve_specific(inputs, filters, biases = None, S = 1, P = 1):
        ...
        print(f"Inputs Shape: {padded_inputs.shape}\nFilters Shape: {filters.shape}\nOutput shape: {output_shape}")
        ...
        for i in range(0, output_shape[0]):
            for j in range(0, output_shape[1]):
                for k, kernel in enumerate(filters):
                    ...
                    outputs_nd[:, i, j, k] = np.sum(inputs_slice * kernel, axis=(1, 2, 3)) + biases[k]
        return outputs_nd
